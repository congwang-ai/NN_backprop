   ...
   /* ...and.... compute the outputs of all the neurons in the output layer */
     ...
     for (j=0; j < network->output_neuron_count; j++)
         { receptive_field = 0.0;
		   ...
		   network->output_layer[j].output = receptive_field;  // Oh look... now it's a linear output neuron!
...
{  int j,k;
   ...
   //   for (j=0; j<network->output_neuron_count; j++)
   //       network->output_layer[j].gradient = network->output_layer[j].output *
   //                                        (1.0 - network->output_layer[j].output) *
   //                                        (d[j] - network->output_layer[j].output);
										   
   /* First, let's do the easy part.  Compute the gradients for all output nodes - in this case, linear */
      for (j=0; j<network->output_neuron_count; j++)
...
{ int j,k;
  ...
  for (j=0; j<network->output_neuron_count; j++)
      { for (k=0; k<network->hidden_neuron_count; k++)
             network->second_layer_weights[j*network->hidden_neuron_count+k] += (learning_rate *
                                                                                 network->output_layer[j].gradient *
                                                                                 network->hidden_layer[k].output);
      
        network->output_layer[j].bias += (learning_rate * network->output_layer[j].gradient * network->hidden_layer[k].output );
      } 

  for (j=0; j<network->hidden_neuron_count; j++)
      { for (k=0; k<network->input_node_count; k++)
             network->first_layer_weights[j*network->input_node_count+k] += (learning_rate *
                                                                                network->hidden_layer[j].gradient *
                                                                                network->input_layer[k]);
    
       network->hidden_layer[j].bias += (learning_rate * network->hidden_layer[j].gradient * network->input_layer[k]);
      }
